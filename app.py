...
if st.session_state.vista_actual == 'general':
    ...
    if datos["diario"]:
        ...
        st.subheader(f"🔥 DISCIPLINA(D): {promedio_disc:.1f}%")      
        fig_disc = go.Figure()
        fig_disc.add_trace(go.Scatter(x=fechas, y=disc_prom, mode='lines+markers', name='Disciplina', 
            ...
            hovertemplate='🔥%{y:.1f}%🔥<br> %{customdata[0]}h<br> de %{customdata[1]}<extra></extra>',
        ...
        st.subheader(f"⚡ VELOCIDAD(V): {promedio_vel:.1f} ejercicios/h")
        fig_vel = go.Figure()
        if vel_prom:
            fig_vel.add_trace(go.Scatter(x=fechas, y=vel_prom, mode='lines+markers', name='Velocidad', 
                line=dict(color='gold', width=3), 
                marker=dict(size=8, color='gold'), 
                hovertemplate='⚡%{y:.1f}ejer/h⚡ ejer/h<br> %{customdata[0]} ejer en %{customdata[1]}h<br> de %{customdata[2]}<extra></extra>',
